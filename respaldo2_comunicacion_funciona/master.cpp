// ==== Heltec WiFi LoRa 32 V3 — MASTER (MODO: Receptor de Sensor) ====
#include <Arduino.h>
#include <SPI.h>
#include <Wire.h>
#include <RadioLib.h>
...
void setup(){
  ...
  delay(200); Serial0.println("\n[MASTER] Receptor de Sensor listo");
  vextOn(); delay(15);
    ...
    if(rs==RADIOLIB_ERR_NONE){
      ...
      if(parseMesh(s,src,dst,ttl,seq,pl) && !seenRecently(src,seq)){
        ...
        if (pl.startsWith("DATA,")) {
            // Es un paquete de datos del sensor. Lo decodificamos.
            ...
            if (secondComma > firstComma) {
                ...
                note = "Error en formato DATA";
            }
        } else {
            // Es otro tipo de mensaje, como el ping original
            note = "Ping: " + pl;
        }
        ...
        if (dst == MASTER_ID) {
          ...
          if (lastHdrLen > 0) {
            ...
            radio.transmit(outBuf, n);
          } else {
            radio.transmit(ackAscii);
          }
        }
      }
    }
